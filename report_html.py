...
<div class="section">
  ...
  <p class="desc">Each principal component's loadings shown as a radar — cupping dimensions (left) and flavor intensity (right). Solid lines = positive correlation; dashed lines = negative correlation.</p>
  <div class="charts-row">
...
(function() {{
  ...
  function makeRadar(chart, dimNames, sliceFn, maxVal, showLegend) {{
    const indicator = dimNames.map(function(d) {{ return {{ name: d, max: maxVal }}; }});
    const series = [];
    loadings.forEach(function(row, i) {{
      const vals = sliceFn(row);
      const posVals = vals.map(function(v) {{ return v > 0 ? v : 0; }});
      const negVals = vals.map(function(v) {{ return v < 0 ? -v : 0; }});
      const pcLabel = 'PCA ' + (i+1);
      const color = colors[i % colors.length];
      series.push({{
        name: pcLabel,
        type: 'radar',
        data: [{{ value: posVals, name: pcLabel + ' (+)' }}],
      ...
      series.push({{
        name: pcLabel,
        type: 'radar',
        data: [{{ value: negVals, name: pcLabel + ' (\u2212)' }}],
    ...
    chart.setOption({{
      tooltip: {{ trigger: 'item' }},
      legend: showLegend ? {{ top: 5, textStyle: {{ color: '#ccc', fontSize: 10 }}, selector: ['all', 'inverse'] }} : {{ show: false }},
      radar: {{ indicator: indicator, shape: 'polygon', splitArea: {{ areaStyle: {{ color: ['#16213e', '#1a2744'] }} }}, axisName: {{ color: '#999', fontSize: 9 }} }},
      series: series
    }});
  }}
